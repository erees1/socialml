...
def filter_array(arr, remove_hyperlinks, remove_words, max_message_length):
    ...
    result = _filter_array_func(arr, remove_hyperlinks, remove_words, max_message_length)

    return result

def _filter_array_func(arr, remove_hyperlinks, remove_words, max_message_length):

    filtered_arr = []
    for conversation in arr:
        filtered_conversation = []
        conversation_has_hyperlinks_present = False
        conversation_has_unwanted_words = False
        ...
        for message in conversation:
            ...
            if _check_filter_conditions(
                ...
                if has_unwanted_words:
                    conversation_has_unwanted_words = True
                if is_over_max_length:
                    conversation_has_messages_over_max_length = True

        if _check_filter_conditions(
            conversation_has_unwanted_words,
            remove_words[0],
